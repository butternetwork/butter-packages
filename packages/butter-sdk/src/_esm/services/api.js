...
export const getTokensForNetwork = async ({ network, page = 1, size = 10, keyword, }) => {
    ...
    const result = await request(`${butterConfig.apiUrl}/api/queryTokenList?` + params);
    return result.data;
};
export const getRoutes = async ({ fromChainId, tokenOutAddress, toChainId, tokenInAddress, amount, type = "exactIn", slippage, entrance = "Butter+", abortSignal, }) => {
    const params = new URLSearchParams();
...
export const getSwapHistory = async ({ page, size, sourceAddress, }) => {
    const result = await request(`${butterConfig.historyApiUrl}/api/queryBridgeHistoryByAddress?page=${page}&size=${size}&address=${sourceAddress}`);
    return result.data;
};
export const getSwapHistoryDetail = async ({ id, }) => {
    const result = await request(`${butterConfig.historyApiUrl}/api/queryBridgeInfoById?id=${id}`);

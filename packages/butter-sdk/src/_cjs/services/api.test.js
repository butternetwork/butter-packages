...
(0, vitest_1.describe)("getTokensForNetwork", () => {
    (0, vitest_1.it)("should return tokens for a valid network", async () => {
        const tokenResult = await (0, api_js_1.getTokensForNetwork)({ network: "ethereum" });
        (0, vitest_1.expect)(tokenResult.count).toBeGreaterThan(0);
        (0, vitest_1.expect)(tokenResult.results.length).toBeGreaterThan(0);
    }, {
        timeout: 30000,
    });
    (0, vitest_1.it)("should handle network with no tokens", async () => {
        const tokenResult = await (0, api_js_1.getTokensForNetwork)({ network: "invalid" });
...
(0, vitest_1.describe)("getRoutes", () => {
    (0, vitest_1.it)("should return routes for a valid network", async () => {
...
(0, vitest_1.describe)("generateSwapData", async () => {
    (0, vitest_1.it)("should return valid tx data", async () => {
        const routes = await (0, api_js_1.getRoutes)({
            fromChainId: "1",
            toChainId: "56",
            amount: "1",

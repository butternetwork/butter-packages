...
const useEvmAllowance = ({ data }) => {
    ...
    const { data: allowanceData, refetch } = useContractRead({
        ...
        args: [wallet?.address, data ? data[0]?.to : ""],
        enabled,
        watch: true,
        staleTime: 1000,
    });
    const needApprove = useMemo(() => {
        if (!isNaN(Number(amount)) && from && allowanceData >= BigInt(0)) {
            const amountBig = parseUnits(amount || "0", from.token?.decimals);
            return amountBig > BigInt(allowanceData);
        }
        else {
            return false;
        }
    ...
    const approve = async (value) => {
        if (BigInt(allowanceData?.toString() || 0) > BigInt(0) && isMainnetUSDT) {

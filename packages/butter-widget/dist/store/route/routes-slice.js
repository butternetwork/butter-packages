...
import { getSwapHistory } from "@butternetwork/sdk";
const initialState = {
    from: null,
    to: null,
    slippage: "2",
...
export const getUserHistory = createAsyncThunk("routes/getUserHistory", async ({ account, page }) => {
    if (!account) {
        return {
            total: 0,
            list: [],
...
export const routesSlices = createSlice({
    ...
    reducers: {
        updateFrom: (state, action) => {
            state.amount = toFixedN(state.amount, action.payload.token?.decimals).toString();
...
export const { updateFrom, updateTo, updateSlippage, updateAmount, updateLoading, updateRoutes, updateInputAddress, updateSelectedRoute, swapComplete, toggleSetting, toggleAddressEditor, toggleShowExchangeModal, setRouteTxData, setCurrentHistoryData, setFetchRouteError, closeRoute, } = routesSlices.actions;

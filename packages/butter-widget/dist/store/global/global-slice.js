...
import { getChains, getTokensForNetwork, } from "@butternetwork/sdk";
const initialState = {
    chains: [],
    metamaskChains: [],
    loadingChains: false,
...
export const fetchChainsData = createAsyncThunk("global/fetchChainsData", async () => {
    const res = await getChains();
    const chains = [...res];
    return chains;
});
const TOKEN_PAGE_SIZE = 10;
export const getTokensForChain = createAsyncThunk("routes/getTokensForChain", async ({ network, page, chainId, wallet, symbol }, { getState }) => {
    if (!network) {
        return {
            tokensForChain: {},
            tokensForChainNoMore: {},
    ...
    try {
        if (!!wallet) {
            const balances = await wallet.getBalances(tokensRes.map((item) => {
                return {
                    address: item.address,
                    decimals: item.decimals,
                };
            }), Number(chainId));
            tokensRes = tokensRes.map((item, index) => {
                return {
                    ...item,
        ...
        tokensRes = tokensRes.map((item, _index) => {
            return {
                ...item,
    ...
    tokensRes = tokensRes.sort((a, b) => Number(a.balance) > Number(b.balance) ? -1 : 1);
...
export const globalSlice = createSlice({
    ...
    initialState,
    reducers: {
        updateTokensPage: (state, action) => {
            state.tokensForChainPages = {
                ...state.tokensForChainPages,
                ...action.payload,
            };
        },
        setShowConfirmCard: (state, action) => {
            state.showConfirmCard = action.payload;
        },
        setTokensNoMoreForNetwork(state, action) {
    ...
    extraReducers: (builder) => {
        builder.addCase(fetchChainsData.fulfilled, (state, action) => {
...
export const { updateTokensPage, setShowConfirmCard, setTokensNoMoreForNetwork, } = globalSlice.actions;

...
import { erc20ABI, useAccount, useChainId, useDisconnect, } from "wagmi";
...
const useEvmWallet = () => {
    ...
    const getBalances = useCallback(async (tokens, chainId) => {
        if (!account.address) {
            return [];
        }
    ...
    const getConnectedWallet = useCallback(() => {
        if (account && account.address && account.connector) {
            return {
                address: account.address,
                connector: account.connector?._wallets?.[0]?.id || account.connector?.id,
                providerName: name,

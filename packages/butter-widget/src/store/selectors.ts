...
export const selectSlippage = memoize(
  (state: RootState) => state.routes.slippage,
);
export const selectAmount = memoize((state: RootState) => state.routes.amount);
export const selectInputAddress = memoize(
  (state: RootState) => state.routes.inputAddress,
);

export const selectShowAddressEditor = memoize(
  (state: RootState) => state.routes.showAddressEditor,
);

export const selectShowSetting = memoize(
  (state: RootState) => state.routes.showSetting,
);
...
export const selectUserHistory = createSelector(
  [
    (_state: RootState, account: string) => account,
    (state: RootState) => state.routes.userHistory,
  ],
  (account, history) => history[account] || [],
);
export const selectUserHistoryTotalCount = (state: RootState) =>
  state.routes.userHistoryTotal;
export const selectUserHistoryForPage = createSelector(
  [
    (_state, page: number, size: number, account: string) => ({
      page,
      size,
      account,
    }),

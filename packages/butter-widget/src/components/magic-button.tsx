...
const useStyles = createStyles(
  (
    theme,
    { width, height, justifyContent, radius, selected }: MagicButtonStyleProps,
  ) => ({
    btn: {
    ...
    btnFill: {
      ...
      transition:
        "0.4s cubic-bezier(0.5, 0.75, 0, 1), border 0.2s ease-in-out, background-color 0.2s ease-in-out",
    },
    btnContent: {
...
const MagicButton = ({
  ...
  onClick,
  selected,
}: PropsWithChildren<MagicButtonProps>) => {
  ...
  return (
    <button className={classes.btn} onClick={onClick}>
      <div className={classes.btnClick}>
        ...
        <div className={classes.btnContent}>{children}</div>
        <div className={classes.btnDuplicateFill}></div>
        <div className={classes.btnDuplicateContent}>{children}</div>
      </div>
    </button>
  );
};
